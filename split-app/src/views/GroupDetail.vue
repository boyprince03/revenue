    ...
    <div class="bg-dark-800 text-white pt-8 pb-12 px-6 rounded-b-[2.5rem] shadow-xl z-10 relative">
      ...
      <div class="text-center -mt-2 mb-6"> 
        ...
        <div class="mt-4 flex flex-col items-center animate-fade-in">
           <span class="text-[10px] uppercase tracking-widest text-gray-400">Total Expenses</span>
      ...
      <div class="flex overflow-x-auto gap-4 py-2 no-scrollbar mt-4 justify-center items-start min-h-[100px]"> 
        <div v-if="members.length > 1"
          ...
          <span class="text-[10px] tracking-wider uppercase">All</span>
        </div>
    ...
    <div class="px-5 -mt-6 relative z-20">
      <div class="flex justify-between items-end mb-4 px-2">
         <h3 class="text-xs uppercase tracking-widest text-gray-400">
           {{ selectedMember === 'all' ? 'All Transactions' : 'Registered by Member' }}
         </h3>
      </div>
                ...
                <div>
                  ...
                  <div class="flex justify-center flex-wrap gap-2 relative">
                    ...
                    <div v-if="hiddenTags.length > 0" class="relative">
                      <span @click.stop="showTagMenu = !showTagMenu" class="px-3 py-1 bg-gray-50 rounded-full text-xs text-gold-600 cursor-pointer font-medium">More...</span>
                 ...
                 <div class="bg-green-50 p-5 rounded-xl text-center">
                    ...
                    <div v-if="transferTarget" class="mt-6 pt-4 border-t border-green-200/50">
                       <div v-if="getMemberBank(transferTarget).hasInfo" class="animate-fade-in">
                          <p class="text-[10px] text-gray-400 uppercase mb-1">Recipient's Account</p>
                       ...
                       <p v-else class="text-xs text-gray-400 italic mt-2">
                         No payment info provided by this user.
                       </p>
                    </div>
        ...
        <transition name="modal-content" appear>
          <div class="bg-white w-full max-w-sm rounded-2xl shadow-xl overflow-hidden flex flex-col max-h-[80vh]">
            <div class="p-5 border-b border-gray-100 flex justify-between items-center bg-gray-50">
              <div>
                <h3 class="font-bold text-dark-800 text-lg">Settlement Plan</h3>
                <p class="text-xs text-gray-400">Suggested payments to clear debts</p>
              </div>
            ...
            <div class="p-4 overflow-y-auto">
               ...
               <div v-else class="space-y-3">
                 <div v-for="(plan, index) in suggestedSettlements" :key="index"
                    ...
                    <div v-if="plan.from === currentUser?.uid" class="z-10 bg-gold-500 text-white text-[10px] uppercase font-bold px-2 py-1 rounded-full shadow-lg">Pay</div>
                    <div v-else-if="plan.to === currentUser?.uid" class="z-10 bg-green-500 text-white text-[10px] uppercase font-bold px-2 py-1 rounded-full shadow-lg">Collect</div>
          ...
          <transition name="modal-content" appear>
             <div class="bg-white w-full max-w-xs rounded-2xl shadow-xl p-6 relative overflow-hidden text-center">
                ...
                <div v-if="selectedPlan.from === currentUser?.uid" class="relative z-10">
                   <h3 class="text-lg font-bold text-dark-800 mb-1">Payment Required</h3>
                   <p class="text-xs text-gray-400 mb-6">You owe <strong class="text-dark-800">{{ getMemberName(selectedPlan.to) }}</strong></p>
                   ...
                   <div class="bg-gray-50 p-4 rounded-xl border border-gray-100 mb-6 text-left">
                      <div v-if="getMemberBank(selectedPlan.to).hasInfo">
                        <p class="text-[10px] uppercase text-gray-400 tracking-widest mb-1">Pay to Bank</p>
                      ...
                      <div v-else class="text-center py-2">
                        <p class="text-xs text-gray-400 italic">This user hasn't set up payment details.</p>
                      </div>
                   </div>
                ...
                <div v-else-if="selectedPlan.to === currentUser?.uid" class="relative z-10">
                   <h3 class="text-lg font-bold text-dark-800 mb-1">Collect Money</h3>
                   <p class="text-xs text-gray-400 mb-6"><strong class="text-dark-800">{{ getMemberName(selectedPlan.from) }}</strong> owes you</p>
                   ...
                   <div class="bg-gray-50 p-4 rounded-xl border border-gray-100 mb-6 text-left">
                      <p class="text-[10px] uppercase text-gray-400 tracking-widest mb-2 text-center border-b border-gray-200 pb-2">Show this to Payer</p>
                      <div v-if="store.userProfile?.bankAccount">
                        <p class="text-[10px] uppercase text-gray-400 tracking-widest mb-1">My Bank</p>
                      ...
                      <div v-else class="text-center py-2">
                        <p class="text-xs text-gray-400 italic mb-2">You haven't set up payment details.</p>
                        <button @click="openBankEdit" class="text-xs text-gold-500 underline">Setup Now</button>
                      </div>
                   </div>
      ...
      <div v-if="showBankDisplay" class="fixed inset-0 bg-dark-900/80 backdrop-blur-sm z-[70] flex items-center justify-center p-6">
        <transition name="modal-content" appear>
           <div class="bg-white w-full max-w-xs rounded-2xl shadow-xl p-6 text-center relative overflow-hidden">
             ...
             <h3 class="text-lg font-bold text-dark-800 mb-1">My Payment Info</h3>
             <p class="text-xs text-gray-400 mb-6">Show this to receive money</p>

             <div class="bg-gray-50 p-4 rounded-xl border border-gray-100 mb-6">
               <p class="text-[10px] uppercase text-gray-400 tracking-widest mb-1">Bank</p>
               <p class="font-medium text-dark-900 mb-3">{{ store.userProfile?.bankName || 'Not Set' }}</p>
               
               <p class="text-[10px] uppercase text-gray-400 tracking-widest mb-1">Account Number</p>
             ...
             <button @click="showBankDisplay = false" class="w-full py-3 bg-dark-800 text-white rounded-xl shadow-lg">Close</button>
           </div>
        </transition>
      </div>
    </transition>
      ...
      <div v-if="showBankEdit" class="fixed inset-0 bg-dark-900/80 backdrop-blur-sm z-[80] flex items-center justify-center p-6">
         <transition name="modal-content" appear>
           <div class="bg-white w-full max-w-xs rounded-2xl shadow-xl p-6">
             <h3 class="text-lg font-bold text-dark-800 mb-4">Setup Payment Info</h3>
             <div class="space-y-4">
                <div>
                  <label class="block text-xs uppercase text-gray-400 tracking-wider mb-1">Bank Name / Code</label>
                ...
                <div>
                  <label class="block text-xs uppercase text-gray-400 tracking-wider mb-1">Account Number</label>
             ...
             <div class="flex gap-3 mt-8">
               <button @click="showBankEdit = false" class="flex-1 py-2 text-gray-400 text-sm">Cancel</button>
               <button @click="saveBankInfo" class="flex-1 py-2 bg-gold-500 text-white rounded-lg shadow-md hover:bg-gold-600">Save</button>
             </div>
           </div>
         </transition>
      </div>
...
const showSettlementInfoModal = ref(false); // New
const selectedPlan = ref(null);             // New
...
const updateMembersList = async (memberIds) => {
  const promises = memberIds.map(async (uid) => {
      ...
      return { 
        uid, 
        nickname: uid === currentUser.uid ? 'Me' : (data.nickname || 'Unknown'),
...
const openBankEdit = () => {
  ...
  showSettlementInfoModal.value = false; // Close settlement info if opening edit from there
  bankForm.value = { 
...
const deleteExpense = async (id) => { if(confirm("Delete this transaction?")) await deleteDoc(doc(db, "expenses", id)); };
</script>
